...
let fs = require('fs')
let raw = fs.readFileSync('./ssData.tsv').toString()
...
let cm = {resourceType:"ConceptMap",id:"canshare-scripted-dec"}
cm.url = "http://canshare.co.nz/fhir/ConceptMap/canshare-scripted-dec"
cm.identifier = {value:"canshare-scripted-dec",system:"http://canshare.co.nz/fhir/NamingSystem/conceptmaps"}
cm.title = "December ConceptMap"
cm.status = "draft"
...
for (const key of Object.keys(hashSource)) {
    ...
    // ie the value of the thing I'm looking for (eg cancer stream) is the target - if the dependncies match
    arLines.forEach(function(ar){
        ...
        let targetConceptId = ar[5]     //F conceptId if targetType is 'Value'
        ...
        let dep1Property = ar[10]    //K the propertyname for the first dependency
        let dep1Op = ar[11]          //L the operator - set to '= . Not used at present
        let dep1Value = ar[12]       //M the actual code value of the dependency
        let dep1Display = ar[13]   //N the name of the dependency

        let dep2Property = ar[14]    //O the propertyname for the secone dependency
        let dep2Op = ar[15]          //P the operator - set to '=
        let dep2Value = ar[16]       //Q the actual code value of the dependency
        let dep2Display = ar[17]   //R the name of the dependency



        //console.log(ar)

        //create the target defined by the line. This could be a ValueSet or a single concept
        //we could check the targetType, but we'll just see if there is a valueSet name or a concept
        ...
        element.target.push(target)     //add the target. ie the value if all the dependencies list up
...
fs.writeFileSync("./canshare-scripted-december.json",JSON.stringify(cm,null,2))
